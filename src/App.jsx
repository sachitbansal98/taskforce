...
const vibrate = (ms = 10) => { try { navigator.vibrate(ms); } catch (e) { /* ignore */ } };
...
const sendNotification = (title, body, tag) => {
  ...
  try { new Notification(title, { body: body, tag: tag, vibrate: [200, 100, 200] }); } catch (e) { /* ignore */ }
};
...
const buildWhatsAppText = (task) => {
  ...
  if (task.notes) msg += "\n📝 " + task.notes;
  msg += "\n🏗️ " + (proj ? proj.label : "General");
  if (task.due) msg += "\n📅 Due: " + fmtDate(task.due);
  msg += "\n⚡ " + (PRIORITIES.find(p => p.id === task.priority) ? PRIORITIES.find(p => p.id === task.priority).label : "Normal") + " Priority";
...
const parseTaskWithAI = async (text) => {
  ...
  try {
    const response = await fetch("/api/parse", {
      ...
      body: JSON.stringify({ text: text, todayStr: todayStr, dayOfWeek: dayOfWeek }),
    });
...
const parseTaskLocally = (text) => {
  ...
  return { title: title || text, notes: "", project: project, priority: priority, due: due };
};
      ...
      <div style={S.app}>
        <div style={{ padding: "24px 20px", overflowY: "auto" }}>
          ...
          {overdue.length > 0 && (
            <div style={{ ...S.bCard, borderLeft: "3px solid #EB5757" }}>
              ...
              {overdue.slice(0, 5).map(t => {
                ...
                return (
                  <div key={t.id} style={S.bItem}>
                    <span style={{ color: proj ? proj.color : "#828282", marginRight: 8 }}>{proj ? proj.icon : "📋"}</span>
                    <span style={{ flex: 1, color: "#E6EDF3" }}>{t.title}</span>
                    <span style={{ color: "#EB5757", fontSize: 12 }}>{fmtDate(t.due)}</span>
                  </div>
                );
              })}
            </div>
              ...
              {todayTasks.map(t => {
                ...
                return (
                  <div key={t.id} style={S.bItem}>
                    <span style={{ color: proj ? proj.color : "#828282", marginRight: 8 }}>{proj ? proj.icon : "📋"}</span>
                    <span style={{ flex: 1, color: "#E6EDF3" }}>{t.title}</span>
                    <span style={{ fontSize: 11, color: "#828282" }}>{proj ? proj.label : ""}</span>
                  </div>
                );
              })}
            </div>
          ...
          {PROJECTS.map(proj => {
            ...
            return (
              <div key={proj.id} style={{ ...S.bCard, borderLeft: "3px solid " + proj.color }}>
                ...
                {pt.slice(0, 3).map(t => (
                  <div key={t.id} style={S.bItem}>
                    <span style={{ flex: 1, color: "#C9D1D9", fontSize: 13 }}>{t.title}</span>
                    {t.due && <span style={{ fontSize: 11, color: isOverdue(t.due) ? "#EB5757" : "#828282" }}>{fmtDate(t.due)}</span>}
                  </div>
                ))}
          ...
          <button style={S.waShareBtn} onClick={() => {
            var l = ["📋 *TaskForce Daily Briefing*\n📅 " + new Date().toLocaleDateString("en-IN", { weekday: "long", day: "numeric", month: "long" }) + "\n"];
            l.push("📊 *" + totalPending + " pending* | *" + overdue.length + " overdue* | *" + todayTasks.length + " today*\n");
            if (overdue.length) {
              l.push("⚠️ *OVERDUE:*");
              overdue.slice(0, 5).forEach(t => { var p = PROJECTS.find(x => x.id === t.project); l.push("  → " + t.title + " (" + (p ? p.label : "") + ")"); });
              l.push("");
            }
            if (todayTasks.length) {
              l.push("📌 *TODAY:*");
              todayTasks.forEach(t => { var p = PROJECTS.find(x => x.id === t.project); l.push("  → " + t.title + " (" + (p ? p.label : "") + ")"); });
            }
        ...
        <div style={{ padding: "24px 20px" }}>
          ...
          <div style={S.sCard}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <button style={{
                ...
                <div style={{ width: 22, height: 22, borderRadius: 11, background: "#fff", position: "absolute", top: 3, left: notifEnabled ? 27 : 3, transition: "left 0.3s", boxShadow: "0 1px 4px rgba(0,0,0,0.25)" }} />
              </button>
            </div>
          </div>
          ...
          <div style={S.sCard}>
            ...
            <div style={{ color: "#4F4F4F", fontSize: 13, lineHeight: 1.5 }}>
              ...
              🎙️ <b style={{ color: "#C9D1D9" }}>Voice:</b> Tap the mic and speak in English.<br /><br />
              <span style={{ color: "#C9D1D9" }}>Call Alwar architect about permits by Friday</span><br />
              <span style={{ color: "#C9D1D9" }}>Get Jhajjar tiles quotation next week</span><br />
              <span style={{ color: "#C9D1D9" }}>Costify dealer follow up tomorrow</span>
            </div>
          </div>

          <div style={{ ...S.sCard, borderColor: "rgba(235,87,87,0.2)" }}>
...
function AIAddScreen({ onAdd, onClose }) {
  ...
  var [manProject, setManProject] = useState(null);
  var [manPriority, setManPriority] = useState(null);
  var [manDue, setManDue] = useState(null);
  var [manTitle, setManTitle] = useState(null);
  var [manNotes, setManNotes] = useState(null);

  useEffect(() => { setTimeout(() => { if (inputRef.current) inputRef.current.focus(); }, 150); }, []);
  ...
  var handleParse = async () => {
    ...
    vibrate(8);
    try {
      var result = await parseTaskWithAI(input.trim());
      if (result && result.title) {
        setParsed(result);
        vibrate([10, 50, 10]);
      } else {
        setParsed(parseTaskLocally(input.trim()));
        setError("AI unavailable — parsed locally");
      }
    } catch (e) {
      setParsed(parseTaskLocally(input.trim()));
      setError("AI unavailable — parsed locally");
  ...
  var handleConfirm = () => {
    ...
    var task = {
      id: genId(),
      title: manTitle !== null ? manTitle : parsed.title,
      notes: manNotes !== null ? manNotes : (parsed.notes || ""),
      project: manProject !== null ? manProject : parsed.project,
      priority: manPriority !== null ? manPriority : parsed.priority,
  ...
  var activeProject = manProject !== null ? manProject : (parsed ? parsed.project : null);
  var activePriority = manPriority !== null ? manPriority : (parsed ? parsed.priority : null);
  var activeDue = manDue !== null ? manDue : (parsed ? parsed.due : null);
  var activeTitle = manTitle !== null ? manTitle : (parsed ? parsed.title : null);
  var activeNotes = manNotes !== null ? manNotes : (parsed ? parsed.notes : null);
    ...
    <div style={S.app}>
      <div style={{ padding: "20px 20px 32px", overflowY: "auto", flex: 1 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", marginBottom: 20 }}>
                <button
                  style={{
                    ...
                    borderRadius: "50%",
                    border: isListening ? "3px solid #EB5757" : "3px solid #1C2333",
                    background: isListening ? "radial-gradient(circle, rgba(235,87,87,0.19), rgba(235,87,87,0.06))" : "linear-gradient(135deg, #161B22, #1C2333)",
                    cursor: "pointer",
                    display: "flex",
                    alignItems: "center",
                    justifyContent: "center",
                    fontSize: isListening ? 40 : 32,
                    transition: "all 0.3s ease",
                    boxShadow: isListening ? "0 0 0 12px rgba(235,87,87,0.07), 0 0 40px rgba(235,87,87,0.13)" : "0 4px 20px rgba(0,0,0,0.25)",
                  }}
                ...
                <div style={{ fontSize: 11, color: "#333D4D", marginTop: 4 }}>🗣️ English (Indian accent supported)</div>
              </div>
            )}

            {isListening && interimText && (
              <div style={{ padding: "12px 16px", background: "rgba(235,87,87,0.06)", border: "1px dashed rgba(235,87,87,0.27)", borderRadius: 12, marginBottom: 12, fontSize: 14, color: "#EB5757", fontStyle: "italic", textAlign: "center" }} className="pulse-text">
                {interimText}...
              </div>
            )}
        ...
        <div style={{ position: "relative", marginBottom: 4 }}>
          <textarea
            ref={inputRef}
            style={{ ...S.aiInput, minHeight: parsed ? 56 : 70, paddingRight: 52, borderColor: isListening ? "rgba(235,87,87,0.27)" : "#1C2333" }}
          ...
          {voiceSupported && !parsed && (
            <button style={{ position: "absolute", right: 10, top: 10, width: 36, height: 36, borderRadius: 10, border: "none", background: isListening ? "rgba(235,87,87,0.19)" : "#1C2333", cursor: "pointer", fontSize: 18, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={toggleVoice}>
              {isListening ? "⏹" : "🎙️"}
            </button>
          )}
        </div>
        ...
        {parsed && (
          <div className="fade-in">
            {error && <div style={{ fontSize: 12, color: "#F2994A", marginBottom: 8, padding: "8px 12px", background: "rgba(242,153,74,0.07)", borderRadius: 8 }}>⚡ {error}</div>}
            <div style={S.previewCard}>
              ...
              <div style={{ marginBottom: 14 }}>
                <div style={S.previewLabel}>Due Date {activeDue ? " — " + fmtDate(activeDue) : ""}</div>
            ...
            <button style={S.retryBtn} onClick={() => { setParsed(null); setError(null); setInput(""); setTimeout(() => { if (inputRef.current) inputRef.current.focus(); }, 100); }}>Try Another</button>
          </div>
        )}
      </div>
    </div>
...
// ─── STYLES ───
var S = {
  app: { minHeight: "100vh", background: "#0D1117", color: "#C9D1D9", fontFamily: "'Outfit','Segoe UI',system-ui,sans-serif", maxWidth: 480, margin: "0 auto", position: "relative", display: "flex", flexDirection: "column" },
  ...
  pill: function(active, color) { return { padding: "8px 16px", borderRadius: 24, fontSize: 13, fontWeight: active ? 600 : 400, border: active ? "1.5px solid " + color : "1.5px solid #1C2333", background: active ? color + "15" : "transparent", color: active ? color : "#4F4F4F", cursor: "pointer", whiteSpace: "nowrap", flexShrink: 0, transition: "all 0.2s", fontFamily: "inherit" }; },
  tCard: function(color) { return { background: "#161B22", borderRadius: 14, padding: "14px 14px", marginBottom: 8, borderLeft: "3px solid " + color, position: "relative", transition: "all 0.2s" }; },
  chk: function(done, color) { return { width: 26, height: 26, borderRadius: 8, border: "none", background: "transparent", cursor: "pointer", padding: 0, flexShrink: 0, marginTop: 1, display: "flex", alignItems: "center", justifyContent: "center" }; },
  chkInner: function(done, color) { return { width: 22, height: 22, borderRadius: 7, border: done ? "none" : "2px solid " + color + "55", background: done ? color : "transparent", color: "#fff", fontSize: 12, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.2s" }; },
  tag: function(color) { return { fontSize: 11, padding: "3px 8px", borderRadius: 6, background: color + "12", color: color, fontWeight: 400 }; },
  waSmall: { width: 32, height: 32, borderRadius: 8, border: "none", background: "rgba(37,211,102,0.09)", cursor: "pointer", fontSize: 16, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 },
  ...
  fab: { position: "fixed", bottom: 24, right: 24, width: 58, height: 58, borderRadius: 18, background: "linear-gradient(135deg, #FF6B35, #F2994A)", color: "#fff", border: "none", cursor: "pointer", boxShadow: "0 4px 20px rgba(255,107,53,0.27), 0 0 0 4px rgba(255,107,53,0.07)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100 },
  ...
  previewCard: { background: "#161B22", borderRadius: 16, padding: "18px 16px", marginBottom: 12, border: "1.5px solid rgba(39,174,96,0.27)" },
  ...
  miniBtn: function(active, color) { return { padding: "7px 12px", borderRadius: 8, fontSize: 12, fontWeight: active ? 600 : 400, border: active ? "1.5px solid " + color : "1.5px solid #1C2333", background: active ? color + "18" : "transparent", color: active ? color : "#4F4F4F", cursor: "pointer", transition: "all 0.15s", fontFamily: "inherit" }; },
  ...
  bItem: { display: "flex", alignItems: "center", gap: 6, padding: "7px 0", borderBottom: "1px solid rgba(28,35,51,0.4)", fontSize: 13 },
  waShareBtn: { width: "100%", padding: "14px", borderRadius: 14, border: "1.5px solid rgba(37,211,102,0.27)", background: "rgba(37,211,102,0.09)", color: "#25D366", fontSize: 15, fontWeight: 600, cursor: "pointer", marginTop: 16, display: "flex", alignItems: "center", justifyContent: "center", gap: 10, fontFamily: "inherit" },
...
style.textContent = [
  ...
  "input:focus, textarea:focus { border-color: rgba(255,107,53,0.53) !important; }",
  ...
  "@keyframes micPulse { 0% { box-shadow: 0 0 0 0px rgba(235,87,87,0.27); } 100% { box-shadow: 0 0 0 20px rgba(235,87,87,0); } }",
